...
(() => {
  ...
  // --- header config (user-editable)
  const HEADER_CONFIG_KEY = 'odoo_helper_headers_v1';
  let headerConfig = [
    { key: 'id', label: 'ID', type: 'number', removable: false },
    { key: 'default_code', label: 'Default Code', type: 'text', removable: false },
    { key: 'name', label: 'Nama', type: 'text', removable: false },
    { key: 'chosenFields', label: 'Fields', type: 'text', removable: false },
    { key: 'qty', label: 'Qty', type: 'number', removable: true },
    { key: 'keterangan', label: 'Keterangan', type: 'text', removable: true }
  ];

  function loadHeaderConfig() {
    try {
      const s = localStorage.getItem(HEADER_CONFIG_KEY);
      if (s) headerConfig = JSON.parse(s);
    } catch (e) { /* ignore */ }
  }
  function saveHeaderConfig() {
    try { localStorage.setItem(HEADER_CONFIG_KEY, JSON.stringify(headerConfig)); } catch(e){}
  }
...
@media (max-width:720px) { #odoo-helper-root { left: 8px !important; right: auto !important; width: calc(100% - 16px); } }
  ...
  const table = el('table', { class: 'odoo-table', id: 'selected-table' });
    // initial thead will be created later after headersConfig is initialized
  ...
  const settingsBox = el('div', { id: 'odoo-helper-settings' },
    ...
    el('div', { style: { marginTop: '10px', borderTop: '1px dashed #eee', paddingTop: '8px' } },
      ...
      el('div', { style: { display: 'flex', gap: '6px', marginTop: '8px' } },
        el('button', { class: 'odoo-btn', id: 'add-header-btn' }, 'Tambah Header'),
        el('button', { class: 'odoo-btn', id: 'save-header-btn' }, 'Simpan Format'),
        el('button', { class: 'odoo-btn', id: 'reset-header-btn' }, 'Reset Format')
      )
    ),
  );
...
function selectiveCleanup(options = {}) {
  ...
  loadHeaderConfig();
  setTimeout(()=>{ try{ renderHeaderManager(); }catch(e){} }, 60);
  ...
  // --- Header Manager render & controls
  function renderHeaderManager() {
    const wrap = document.getElementById('header-manager-list');
    if (!wrap) return;
    wrap.innerHTML = '';
    headerConfig.forEach((h, idx) => {
      const item = el('div', { class: 'hdr-item', style: { display: 'flex', gap: '6px', alignItems: 'center', padding: '6px', borderBottom: '1px solid #f1f1f1' }, draggable: true, 'data-idx': idx },
        el('span', { style: { cursor: 'grab' } }, '≡'),
        el('input', { type: 'text', value: h.label, style: { flex: '1' }, onchange: (e)=>{ h.label = e.target.value; } }),
        el('select', { onchange: (e)=>{ h.type = e.target.value; } },
          el('option', { value: 'text', selected: h.type==='text' }, 'text'),
          el('option', { value: 'number', selected: h.type==='number' }, 'number')
        ),
        el('button', { class: 'odoo-btn', onclick: ()=>{ if (idx<=0) return; const tmp = headerConfig[idx-1]; headerConfig[idx-1]=headerConfig[idx]; headerConfig[idx]=tmp; renderHeaderManager(); renderSelectedTable(); } }, '↑'),
        el('button', { class: 'odoo-btn', onclick: ()=>{ if (idx>=headerConfig.length-1) return; const tmp = headerConfig[idx+1]; headerConfig[idx+1]=headerConfig[idx]; headerConfig[idx]=tmp; renderHeaderManager(); renderSelectedTable(); } }, '↓'),
        h.removable ? el('button', { class: 'odoo-btn', onclick: ()=>{ headerConfig.splice(idx,1); renderHeaderManager(); renderSelectedTable(); } }, 'Hapus') : el('div', { style: { width: '56px' } })
      );
      item.addEventListener('dragstart', (ev)=>{ ev.dataTransfer.setData('text/plain', idx); }, { passive: true });
      item.addEventListener('dragover', (ev)=>{ ev.preventDefault(); }, { passive: false });
      item.addEventListener('drop', (ev)=>{ ev.preventDefault(); try { const from = parseInt(ev.dataTransfer.getData('text/plain'),10); const to = parseInt(item.getAttribute('data-idx'),10); if (!isNaN(from) && !isNaN(to) && from !== to) { const tmp = headerConfig.splice(from,1)[0]; headerConfig.splice(to,0,tmp); renderHeaderManager(); renderSelectedTable(); } } catch(e){} }, { passive: false });
  ...
  setTimeout(()=>{ try{
    ...
    if (addBtn) addBtn.addEventListener('click', ()=>{
      const count = headerConfig.filter(h=>h.key && h.key.startsWith('custom_')).length + 1;
      const key = `custom_${Date.now()}_${count}`;
      headerConfig.push({ key, label: 'Custom', type: 'text', removable: true });
      renderHeaderManager();
      renderSelectedTable();
    });
    const saveBtn = document.getElementById('save-header-btn');
    if (saveBtn) saveBtn.addEventListener('click', ()=>{ saveHeaderConfig(); alert('Format header disimpan'); });
...
function renderSelectedTable() {
  // rebuild thead based on headerConfig
  const oldThead = table.querySelector('thead');
  if (oldThead) oldThead.remove();
  const newThead = el('thead');
  const headTr = el('tr');
  headerConfig.forEach(h => {
    const th = el('th', {}, h.label || h.key);
    headTr.appendChild(th);
  });
  // aksi column (hapus) - always add action column at end
  headTr.appendChild(el('th', {}, 'Aksi'));
  newThead.appendChild(headTr);
  table.appendChild(newThead);

  tbody.innerHTML = '';
  selected.forEach((s, idx) => {
    // ensure s.values exists to store custom inputs
    s.values = s.values || {};
    const rec = s.record;
    const tr = el('tr', { 'data-id': rec.id });

    // for each header in headerConfig, create cell
    headerConfig.forEach(h => {
      let cell;
      const key = h.key;
      ...
      // built-in cases
      if (key === 'id') {
        cell = el('td', {}, String(rec.id || ''));
      } else if (key === 'default_code') {
        cell = el('td', {}, rec.default_code ? String(rec.default_code) : '');
      } else if (key === 'name') {
        cell = el('td', {}, rec.name || '');
      } else if (key === 'chosenFields') {
        // show chosenFields summary
        const otherFields = chosenFields
          .filter(f => !['default_code','name'].includes(f))
          .map(f => {
            const v = rec[f];
            if (v == null) return '';
            if (Array.isArray(v)) return `${availableFields && availableFields[f] ? availableFields[f].string : f}: ${v[1]}`;
            return `${availableFields && availableFields[f] ? availableFields[f].string : f}: ${v}`;
          }).filter(Boolean).join(' • ');
        cell = el('td', {}, otherFields);
      } else {
        // for other keys: if record has that field, display value (read-only)
        if (rec.hasOwnProperty(key)) {
          const v = rec[key];
          const display = Array.isArray(v) ? (v[1] || '') : (v == null ? '' : String(v));
          cell = el('td', {}, display);
        } else {
          // otherwise treat as input column (custom header or qty/keterangan)
          const inputType = (h.type === 'number') ? 'number' : 'text';
          const val = (key === 'qty') ? (s.qty || '') :
                      (key === 'keterangan') ? (s.note || '') :
                      (s.values[key] || '');
          const input = el('input', { type: inputType, value: val, style: { width: '120px' } });
          input.addEventListener('input', (e) => {
            const v = e.target.value;
            if (key === 'qty') s.qty = v;
            else if (key === 'keterangan') s.note = v;
            else s.values[key] = v;
          });
          cell = el('td', {}, input);
        }
    ...
    // aksi (hapus)
    const btn = el('button', { class: 'odoo-btn', onclick: ()=>{ selected = selected.filter(x=>x.record.id!==rec.id); renderSelectedTable(); } }, 'Hapus');
  ...
  function csvEscape(v){ if (v==null) return ''; const s = String(v).replace(/"/g,'""'); if (s.includes(',')||s.includes('"')||s.includes('\n')) return '"' + s + '"'; return s; }

  function downloadCSV() {
  if (!selected.length) return alert('Daftar kosong');
  const headersLabel = headerConfig.map(h => h.label || h.key);
  const headersKey = headerConfig.map(h => h.key);
  const rows = [ headersLabel.map(csvEscape).join(',') ];

  selected.forEach(s => {
    const rec = s.record;
    const line = headersKey.map(h => {
      if (h === 'id') return rec.id;
      if (h === 'qty') return s.qty || '';
      if (h === 'keterangan') return s.note || '';
      if (h === 'chosenFields') {
        return chosenFields.filter(f => !['default_code','name'].includes(f)).map(f => {
          const v = rec[f];
          if (Array.isArray(v)) return v[1] || '';
          return (v == null) ? '' : String(v);
        }).join(' • ');
      }
      if (s.values && (s.values[h] !== undefined)) return s.values[h];
      const v = rec[h];
      if (Array.isArray(v)) return v[1] || '';
      return (v == null) ? '' : String(v);
    }).map(csvEscape).join(',');
  ...
  // gunakan newline actual (bukan literal '\n')
  const blob = new Blob([rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `odoo_selected_${(new Date()).toISOString().slice(0,19).replace(/[:T]/g,'-')}.csv`;
  ...
  // helper: buang tab/newline di dalam cell supaya paste ke Excel rapi
  function safeForClipboard(v) {
    if (v == null) return '';
  ...
  selected.forEach(s => {
    ...
    const line = headersKey.map(h => {
      let out;
      if (h === 'id') out = rec.id;
      else if (h === 'qty') out = s.qty || '';
      else if (h === 'keterangan') out = s.note || '';
      else if (h === 'chosenFields') {
        out = chosenFields.filter(f => !['default_code','name'].includes(f)).map(f => {
          const v = rec[f];
          if (Array.isArray(v)) return v[1] || '';
          return (v == null) ? '' : String(v);
        }).join(' • ');
      } else if (s.values && (s.values[h] !== undefined)) out = s.values[h];
      else {
        const v = rec[h];
        if (Array.isArray(v)) out = v[1] || '';
        else out = (v == null) ? '' : String(v);
      }
      return safeForClipboard(out);
    }).join('\t');
    lines.push(line);
  });
  ...
  navigator.clipboard.writeText(lines.join('\n')).then(
    () => alert('Disalin ke clipboard. Paste di Excel.'),
    (err) => { console.error(err); alert('Gagal copy clipboard'); }
  );
  }
