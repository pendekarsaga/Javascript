// Odoo Product Helper v4.3
// - Adds: tokens multiEntry index, worker-based in-memory search, UI: suggestion limit, debounce ms, db count, settings gear.
// Paste as one block into browser Console (desktop / Android).
(() => {
  if (window.__odoo_tool_injected_v4_3_full) { console.log('Odoo Product Helper: already injected (v4.3)'); return; }
  window.__odoo_tool_injected_v4_3_full = true;
  ...
  // --- styles (override blur + UI) - mostly kept from v4.2 with small additions
...
@media (max-width:720px) { #odoo-helper-root { left: 8px !important; right: auto !important; width: calc(100% - 16px); } }
  ...
  const btnMin = el('button', { class: 'hdr-btn', onclick: toggleMinimize, title: 'Minimize' }, '▁');
  const btnMax = el('button', { class: 'hdr-btn', onclick: toggleMaximize, title: 'Maximize' }, '⬜');
  const btnClose = el('button', { class: 'hdr-btn', onclick: () => root.remove(), title: 'Close' }, '✕');
  const btnGear = el('button', { class: 'hdr-btn', title: 'Settings', onclick: toggleSettings }, '⚙');
  ...
  (function makeDraggable(headerEl, containerEl) {
    ...
    headerEl.addEventListener('pointerdown', start, { passive: false });
  ...
  (function makeResizable(handleEl, containerEl) {
    ...
    handleEl.addEventListener('pointerdown', start, { passive: false });
    window.addEventListener('pointermove', move, { passive: false });
    window.addEventListener('pointerup', end, { passive: false });
  ...
  // export CSV from IndexedDB (with segmented option)
  ...
  function createWorker() {
    ...
    const workerCode = `
      ...
      function build(all) {
        ...
        // convert sets to arrays for easier postMessage (but we keep sets internally)
        // No conversion necessary for internal; but for postMessage we send products only
      ...
      function search(q, limit) {
        ...
        for (const part of qs) {
          // find tokens in index that startWith part
          const matchedIds = [];
        ...
        // rank by simple score: how many query tokens appear in product.search_text
  ...
  async function rebuildWorkerFromDB() {
    ...
    try {
      ...
      // ensure tokens exist on each product (they should from saveProductsBatch)
      const arr = all.map(p => {
      ...
      // keep a small in-memory cache copy (optional) for quick fallback in small DBs
  ...
  console.log('Odoo Product Helper injected (v4.3) - tokens index + worker search + settings UI added.');
})();
