...
@media (max-width:720px) { #odoo-helper-root { left: 8px !important; right: auto !important; width: calc(100% - 16px); } }
  ...
  // dynamic thead builder (replace static thead)
  function buildTableHead() {
    const tr = el('tr', {});
    tr.appendChild(el('th', {}, '#'));
    headersConfig.forEach(h => {
      if (h.key === '__chosen_fields__') {
        tr.appendChild(el('th', {}, h.label || 'chosenFields'));
      } else {
        tr.appendChild(el('th', {}, h.label || h.key));
      }
    });
    tr.appendChild(el('th', {}, 'Aksi'));
    return el('thead', {}, tr);
  }
  ...
  function rebuildTableHead() {
    try {
      ...
      const newHead = buildTableHead();
      table.insertBefore(newHead, table.querySelector('tbody'));
    } catch (e) { console.warn('rebuildTableHead failed', e); }
  }
  ...
  headersConfig.forEach(h => {
    if (h.key === '__chosen_fields__') {
      chosenFields.forEach(cf => out.push(cf));
    } else {
      out.push(h.key);
    }
  });
  // optional: ensure id is first column
  const idxId = out.indexOf('id');
  if (idxId > -1) { out.splice(idxId,1); out.unshift('id'); }
  return out;
  }
  ...
  // --- Header Config UI (inside settingsBox)
  const headerConfigSection = el('div', { style: { marginTop: '8px', borderTop: '1px dashed #eee', paddingTop: '8px' } },
    el('div', { class: 'odoo-small' }, 'Pengaturan Header (Daftar Produk Terpilih):'),
    el('div', { id: 'header-config-list', style: { marginTop: '6px', maxHeight: '160px', overflow: 'auto' } }),
    el('div', { style: { display: 'flex', gap: '6px', marginTop: '6px' } },
      el('button', { class: 'odoo-btn', id: 'btn-add-header' }, 'Tambah Header'),
      el('button', { class: 'odoo-btn', id: 'btn-save-header' }, 'Simpan Format'),
      el('button', { class: 'odoo-btn', id: 'btn-reset-header' }, 'Reset Format')
    ),
    el('div', { class: 'odoo-small', style: { marginTop: '6px', color: '#555' } }, 'Seret item untuk mengubah urutan. Header fixed tidak bisa dihapus.')
  );
  settingsBox.appendChild(headerConfigSection);
  
  function renderHeadersUI() {
    const wrap = document.getElementById('header-config-list');
    if (!wrap) return;
    wrap.innerHTML = '';
    headersConfig.forEach((h, idx) => {
      const item = el('div', { class: 'odoo-small', draggable: true, style: { padding: '6px', border: '1px solid #eee', marginBottom: '6px', display: 'flex', gap: '8px', alignItems: 'center', background: '#fff' }, 'data-idx': idx },
        el('span', { style: { cursor: 'grab', paddingRight: '6px' } }, '☰'),
        el('strong', {}, h.label || h.key),
        el('span', { style: { fontSize: '12px', color: '#666', marginLeft: '6px' } }, `(${h.type})`),
        h.fixed ? el('span', { style: { marginLeft: 'auto', fontSize: '12px', color: '#999' } }, 'fixed') : el('button', { class: 'odoo-btn', style: { marginLeft: 'auto' }, onclick: ()=>{ headersConfig.splice(idx,1); saveHeadersConfig(); renderHeadersUI(); rebuildTableHead(); renderSelectedTable(); } }, 'Hapus')
      );
      // DnD handlers
      item.addEventListener('dragstart', (ev) => { ev.dataTransfer.setData('text/plain', String(idx)); });
      ...
      item.addEventListener('drop', (ev) => {
        ...
        if (!isNaN(from) && !isNaN(to) && from !== to) {
          const sp = headersConfig.splice(from,1);
          headersConfig.splice(to,0, sp[0] );
          saveHeadersConfig(); renderHeadersUI(); rebuildTableHead(); renderSelectedTable();
        }
      });
  ...
  document.getElementById('btn-add-header').addEventListener('click', () => {
    const label = prompt('Nama header (contoh: catatan_vendor):');
    if (!label) return;
    const type = prompt('Tipe kolom: text atau number', 'text');
    const key = label.trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g,'');
    if (headersConfig.some(h=>h.key === key)) return alert('Key sudah ada, pilih nama lain.');
    headersConfig.push({ key, label: label.trim(), type: (type === 'number' ? 'number' : 'text'), fixed: false });
    saveHeadersConfig(); renderHeadersUI(); rebuildTableHead(); renderSelectedTable();
  });
  document.getElementById('btn-save-header').addEventListener('click', ()=>{ saveHeadersConfig(); alert('Format header disimpan.'); });
  document.getElementById('btn-reset-header').addEventListener('click', ()=>{ if (!confirm('Reset format header ke default?')) return; resetHeadersConfig(); alert('Format header dikembalikan ke default.'); });
...
function selectiveCleanup(options = {}) {
  ...
  // --- dynamic headers config (saveable)
  const PRESET_HEADERS_KEY = 'odoo_helper_headers_v1';

  function defaultHeaders() {
  return [
  { key: 'id', label: 'id', type: 'number', fixed: true },
  { key: 'default_code', label: 'default_code', type: 'text', fixed: true },
  { key: 'name', label: 'name', type: 'text', fixed: true },
  { key: '__chosen_fields__', label: 'chosenFields', type: 'group', fixed: true },
  { key: 'qty', label: 'qty', type: 'number', fixed: false },
  { key: 'keterangan', label: 'keterangan', type: 'text', fixed: false }
  ];
  }

  let headersConfig = (function(){
  try {
  const saved = localStorage.getItem(PRESET_HEADERS_KEY);
  if (saved) {
  ...
  function saveHeadersConfig() {
  try { localStorage.setItem(PRESET_HEADERS_KEY, JSON.stringify(headersConfig)); } catch(e){ console.warn('saveHeadersConfig failed', e); }
  }
  function resetHeadersConfig() {
  headersConfig = defaultHeaders();
  saveHeadersConfig();
  try { renderHeadersUI(); } catch(e){}
  try { rebuildTableHead(); renderSelectedTable(); } catch(e){}
  }

  // build initial thead safely (after headersConfig exists)
  try {
    if (typeof table !== 'undefined') {
      const tbodyEl = table.querySelector('tbody');
      if (tbodyEl) table.insertBefore(buildTableHead(), tbodyEl);
      else table.appendChild(buildTableHead());
    }
  } catch (e) {
    console.warn('init thead failed', e);
  }
  ...
  function addSelected(rec, focusQty=false) {
    ...
    selected.push({ record: rec, qty: '', note: '', custom: {} });
    renderSelectedTable();
    setTimeout(()=>{
      const row = tbody.querySelector(`[data-id="${rec.id}"]`);
  ...
  selected.forEach((s, idx) => {
    ...
    headersConfig.forEach(h => {
      if (h.key === '__chosen_fields__') {
        ...
        chosenFields.forEach(cf => {
          const td = el('td', {});
          let val = '';
          if (s.custom && Object.prototype.hasOwnProperty.call(s.custom, cf)) {
            val = s.custom[cf];
          } else if (rec && rec[cf] != null) {
            val = Array.isArray(rec[cf]) ? (rec[cf][1] || '') : rec[cf];
          }
          td.appendChild(document.createTextNode(val == null ? '' : String(val)));
          tr.appendChild(td);
        });
      } else {
        const td = el('td', {});
        // get current value (custom override > rec[field] > '')
        let currentVal = '';
        if (s.custom && Object.prototype.hasOwnProperty.call(s.custom, h.key)) {
          currentVal = s.custom[h.key];
        } else if (rec && rec[h.key] != null) {
          currentVal = Array.isArray(rec[h.key]) ? (rec[h.key][1] || '') : rec[h.key];
        } else {
          currentVal = '';
        }

        if (h.type === 'number') {
          const input = el('input', { type: 'number', value: currentVal, style: { width: '90px' } });
          input.addEventListener('input', e => { s.custom = s.custom || {}; s.custom[h.key] = e.target.value; });
          td.appendChild(input);
        } else if (h.type === 'text') {
          const input = el('input', { type: 'text', value: currentVal, style: { width: '140px' } });
          input.addEventListener('input', e => { s.custom = s.custom || {}; s.custom[h.key] = e.target.value; });
          td.appendChild(input);
        } else {
          td.appendChild(document.createTextNode(currentVal == null ? '' : String(currentVal)));
        }
        tr.appendChild(td);
      }
    });
    ...
    // action column
    const aksiTd = el('td', {},
      el('button', { class: 'odoo-btn', onclick: () => { selected = selected.filter(x => x.record.id !== rec.id); renderSelectedTable(); } }, 'Hapus')
    );
  ...
  selected.forEach(s=>{
    ...
    const line = headers.map(h=>{
      // priority: s.custom[h] > rec[h] > ''
      if (s.custom && Object.prototype.hasOwnProperty.call(s.custom, h)) return s.custom[h];
  ...
  (navigator.clipboard && navigator.clipboard.writeText
    ? navigator.clipboard.writeText(text).then(()=>alert('Disalin ke clipboard. Paste di Excel.'), (err)=>{ console.error(err); alert('Gagal copy clipboard'); })
    : (function(){ const ta = document.createElement('textarea'); ta.value = text; document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); alert('Disalin ke clipboard (fallback)'); } catch(e){ alert('Gagal copy clipboard'); } ta.remove(); })()
  );
  }
