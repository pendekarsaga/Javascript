...
function selectiveCleanup(options = {}) {
  ...
  return [...defaults, ...chosenCols, ...tail];
  }
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(arr) && arr.length) { headersConfig = arr; return; }
    }
  } catch(e){}
  ...
  try {
    localStorage.setItem(PRESET_HEADER_KEY, JSON.stringify(headersConfig));
    alert('Format header disimpan');
  } catch(e){ console.error(e); alert('Gagal menyimpan format header'); }
  }
