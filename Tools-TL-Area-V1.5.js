// Odoo Product Helper v4.3 (modified) - includes a clean Close/Exit that removes listeners, worker, globals, style.
(() => {
...
@media (max-width:720px) { #odoo-helper-root { left: 8px !important; right: auto !important; width: calc(100% - 16px); } }
  ...
  function cleanupOdooHelper() {
    try {
      ...
      try { delete window.__odoo_tool_injected_v4_3_full; } catch(e){ window.__odoo_tool_injected_v4_3_full = false; }

      console.log('Odoo Product Helper: cleaned up and removed.');
    } catch (err) {
